...
def general_load_bucket(npart,gbar,delg,iopt,Ns,coopLength,resWavelength,particle_position,s_steps,dels,hist_steps):
    ...
    if particle_position is None:
        ...
        thet_init=make_theta(npart,iopt,N_input,Ns,s_steps)
        gam_init=make_gamma(gam_step,s_steps,npart,hist_steps)
...
def make_theta(n,iopt,N_input,Ns,s_steps):
    '''
    random initialization of the beam load_bucket
    inputs:
    n               # n-macro-particles per bucket
    iopt            # 5=SASE, 4=seeded
    N_input         # Input number of particles in different intervals along the beam
    Ns              # N electrons per s-slice at maximum current
    s_steps         # n-sample points along bunch length
    outputs:
    thet_init       # all buckets macro particles position
    ...
    if iopt==5:
        ...
        for k in range(s_steps):
            N_real=N_input[k]/np.max(N_input)*Ns
            effnoise = np.sqrt(3*M/(N_real/nb))    # Penman algorithm for Ns/nb >> M
            for i in range(nb):       
                for j in range(M):
...
def make_gamma(gam_step,s_steps,npart,hist_steps):
    '''
    ...
    if np.min(gam_step)==np.max(gam_step):
        ...
        gam_hist=np.zeros((s_steps,hist_steps))
        for j in range(s_steps):
        ...
        gam_cdf=np.concatenate((np.zeros((s_steps,1)),gam_cdf),axis=1)

        #make gamma
        x=np.random.rand(s_steps,npart)
        gam_init=np.zeros((s_steps,npart))
        for j in range(s_steps):
