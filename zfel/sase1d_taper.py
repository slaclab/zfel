...
def sase(inp_struct):
   ...
   ## Check if tapered K is given ## 
    if type(unduK)==int:
        print('Tapering off')
        tapering = 0
    else:
        if len(unduK) != z_steps:
            print('unduK length does not equal z steps.')
            if len(unduK) == 1:
                print('No taper, adjusting unduK')
                unduK = np.ones(z_steps)*unduK
                tapering = 0
        elif len(unduK) == z_steps:
            print('Tapering on')
            tapering = 1
    ...
    kappa_1 =(unduK*unduJJ)*e/4/epsilon_0/gamma0
    chi_1   =(unduK*unduJJ)*e/(2*gamma0**2*mc2*e)
        

    density=currentMax/(e*c*2*np.pi*sigmaX2)                # electron density  units: (1/m)
    ...
    resWavelength = unduPeriod*(1+np.power(unduK,2)/2.0)\
    ...
    E02   = density*kappa_1[0]*P0*1E-9/Pbeam/chi_1[0]       # scaled input power
    gbar  = (resWavelength[0]-radWavelength)\
            /(radWavelength)                                # scaled detune parameter
    delg  = eSpread           
    Ns    = currentMax*unduL/unduPeriod/z_steps\
            *resWavelength[0]/c/e                           # N electrons per s-slice [ ]
    ...
    if iopt==5: 
        ...
        for k in range(s_steps):
            ...
            for j in range(z_steps):                                            # evolve e and gamma in s and t by leap-frog
                ...
                gam[:,j+1] = gam[:,j]-2*chi_1[j]*Erhalf*np.cos(thethalf[:,j+1])*delt\
                             +2*chi_1[j]*Eihalf*np.sin(thethalf[:,j+1])*delt
        ...
        #converting scaled electric field, a, and gamma to field, power and gamavg
        ...
        for j in range(z_steps):
            ...
            field = (Er[:,z_steps]+Ei[:,z_steps]*1j)*np.sqrt(chi_1[j]/(density*kappa_1[j])*Pbeam)
            field_s = (Er[:,:]+Ei[:,:]*1j)*np.sqrt(chi_1[j]/(density*kappa_1[j])*Pbeam)
