...
IN.general.Undulator_Z_steps_ratio  = 4;        % integration steps = Undulator_Z_steps_ratio*length_meters +1
...
IN.general.simulate_only_firstsection = 0;      % 1 simulation stops before chicane ; 2 also simulates chicane ; 0 simulates also second section
...
IN.ebeam_bothund.meanenergy          = 6.5054e+03; % electron energy [MeV] center
IN.ebeam_bothund.energy_deviation_rms = 0;  % each shot has a random energy using this rms from center
IN.ebeam_bothund.eSpread = 0.6e-4;  %uncorrelated slice energy spread
IN.ebeam_bothund.emitN   = 0.7e-6;		% normalized transverse emittance [mm-mrad]
IN.ebeam_bothund.currentMax = 4500.0;					% peak current [Ampere]
...
IN.ebeam_bothund.radWavelength = 6.5943e-10;     		% seed wavelength [meter] or SASE spectral plot center wavelength 
IN.ebeam_bothund.bunchShape =1;                         % 1 is smoothed flattop, 2 is Gaussian, 3 is Constant, 4 real flat top
IN.ebeam_bothund.bunchLength = 2.4e-6;                    % half width for flattop, rms for gaussian [m] 
...
IN.ebeam_bothund.fix_energy_for_s_at=6.5054e+03;
...
IN.firstsec.unduL = 20;							% length of undulator [meter]
IN.firstsec.zstart=inf;                             % starting point of taper
IN.firstsec.alpha=0;% 0.010;                        % taper coefficient

IN.secondsec.unduL =20;							% length of undulator [meter]
IN.secondsec.zstart=inf;                             % starting point of taper
IN.secondsec.alpha=0; % 280.0e-3;                        % taper coefficient
...
IN.chicaneandcrystal.Use_Crystal=0;
...
IN.firstsec.Egain=ones(size(IN.firstsec.Egain))*(-25);
IN.secondsec.Egain=ones(size(IN.firstsec.Egain))*(+25);
...
figure
plot(OUTPUT{1}.secondsection.Orel*(1230/(IN.ebeam_bothund.radWavelength*10^9)),abs(OUTPUT{1}.secondsection.fieldFFT_z(:,end)).^2)
xlabel('Energy')

figure
plot(OUTPUT{1}.secondsection.s/10^6/(3*10^8)*10^15,OUTPUT{1}.secondsection.field_s(:,end).*conj(OUTPUT{1}.secondsection.field_s(:,end)))
xlabel('Time')
